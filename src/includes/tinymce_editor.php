...
<script>
    tinymce.init({
        ...
        setup: function(editor) {
            editor.ui.registry.addButton('mobileUpload', {
                icon: 'image', // Usamos el icono de imagen nativo
                tooltip: 'Subir imagen (Galería)',
                onAction: function() {
                    // Al hacer clic, disparamos el input oculto
    ...
    function uploadFileAndInsert(file, editor){
        ...
        editor.insertContent('<p id="loading-img"><em>Subiendo imagen...</em></p>');
        ...
        .then(result => {
            // Eliminar el texto de "Subiendo..."
            const loadingNode = editor.dom.select('#loading-img')[0];
            if(loadingNode) editor.dom.remove(loadingNode);

            if (result && result.location) {
                // INSERTAR LA IMAGEN DIRECTAMENTE
                const imgHtml = `<img src="${result.location}" alt="Imagen subida" style="max-width: 100%; height: auto;">`;
                editor.insertContent(imgHtml);
            } else {
                alert('Error: El servidor no devolvió la URL.');
            }
        })
        .catch(error => {
